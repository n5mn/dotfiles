...
# see zprof in bottom of the file
# zmodload zsh/zprof
...
if [ -f ~/dotfiles/.config/zsh/custom.zsh ]; then
	. ~/dotfiles/.config/zsh/custom.zsh
fi

# uwsm auto start 
if [ "$(tty)" = "/dev/tty1" ] && uwsm check may-start > /dev/null; then
...
ZSH_COMPDUMP_DIR="${XDG_CACHE_HOME:-$HOME/.cache}"
ZSH_COMPDUMP="${ZSH_COMPDUMP_DIR}/zcompdump"
